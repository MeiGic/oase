...
abstract class Controller{
    ...
    public final function showView($viewname = NULL , $controllerName = NULL){
        ...
        $viewpath = ROOT . '/app/View/' . $viewname;
        if(file_exists($viewpath . '.php')){
            $this->_view->init( $viewpath . '.php' , $this->_opdata);
        }else if(file_exists($viewpath . '.html')){
            $this->_view->init( $viewpath .'.html' , $this->_opdata);
        }else if(file_exists($viewpath . '.htm')){
            $this->_view->init( $viewpath .'.htm' , $this->_opdata);
        }else {
            Log::write("View " . $viewpath . "Not found." , 3);
